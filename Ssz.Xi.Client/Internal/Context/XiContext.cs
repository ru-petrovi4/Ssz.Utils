...
namespace Ssz.Xi.Client.Internal.Context
{
    ...
    internal partial class XiContext
    {
        ...
        /// </returns>
        public XiContext(XiServerInfo xiServerInfo, ServiceEndpoint resourceManagementServiceEndpoint,
            uint contextTimeout,
            ...
            try
            {
                ...
                EndpointConfiguration? resourceManagementEndpointConfig =
                    _xiServerInfo.ServerEntry.EndpointServerSettings?.FirstOrDefault(
                        epc =>
                            (
                                epc.ContractType == resourceManagementServiceEndpoint.Contract.Name)
                                // TODO
                             //&&                             
                             //(new Uri(epc.EndpointUrl) ==
                ...
                resourceManagementServiceEndpoint.Binding.ReceiveTimeout = ReceiveTimeout;
                resourceManagementServiceEndpoint.Binding.SendTimeout = _sendTimeout;

                /*                
                ((NetTcpBinding)resourceManagementServiceEndpoint.Binding).Security.Mode = SecurityMode.None;
                ((NetTcpBinding)resourceManagementServiceEndpoint.Binding).Security.Transport.ClientCredentialType = TcpClientCredentialType.Windows;
                ((NetTcpBinding)resourceManagementServiceEndpoint.Binding).Security.Transport.ProtectionLevel = ProtectionLevel.EncryptAndSign;
                ((NetTcpBinding)resourceManagementServiceEndpoint.Binding).Security.Message.ClientCredentialType = MessageCredentialType.Windows;*/

                var iResourceManagementChannelFactory =
                    new ChannelFactory<IResourceManagement>(resourceManagementServiceEndpoint.Binding,
                        resourceManagementServiceEndpoint.Address);

                iResourceManagementChannelFactory.Credentials.Windows.AllowedImpersonationLevel =
                    TokenImpersonationLevel.Delegation;
                XiEndpointRoot.SetMaxItemsInObjectGraph(iResourceManagementChannelFactory.Endpoint.Contract.Operations,
                    resourceManagementEndpointConfig.MaxItemsInObjectGraph);

                /*                
                // Basic HTTP requires the user info that is not Windows credentials, so get it here
                if (sepResourceManagement.Binding.GetType() == typeof(BasicHttpBinding))
                {
                    var bnd = (BasicHttpBinding)sepResourceManagement.Binding;
                    if ((bnd.Security.Mode == BasicHttpSecurityMode.Transport) ||
                        (bnd.Security.Mode == BasicHttpSecurityMode.TransportCredentialOnly))
                    {
                        _xiUserInfo = XiClientCredentials.GetXiUserInfo(encode);
                        if (null == _xiUserInfo) throw new XiClientBaseCancelException("User Cancellation");
                        XiClientCredentials.SetClientCredentials(UserData, sepResourceManagement,
                                                                    cfIResourceManagement.Credentials, _xiUserInfo);
                    }
                }*/
                ...
                _connectedResourceManagementServiceEndpoint = resourceManagementServiceEndpoint;
            }            
            catch
            {
                _contextId = null;
        ...
        private void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                ...
                if (_iResourceManagement != null)
                {
                    ...
                    try
                    {
                        ChannelCloser.Close((ICommunicationObject) _iResourceManagement);
                    }
                    catch
                    {
                    }
                    ...
                    try
                    {
                        ((IDisposable) _iResourceManagement).Dispose();
                    }
                    catch
                    {
                    }
        ...
        public void KeepContextAlive(DateTime nowUtc)
        {
            ...
            if (bKeepAliveIntervalExpired || bCommunicationsFail)
            {
        ...
        {
            get { return _connectedResourceManagementServiceEndpoint.Binding.GetType().ToString(); }
        }
        ...
        /// </summary>
        public ServiceEndpoint ResourceManagementServiceEndpoint
        {
            get { return _connectedResourceManagementServiceEndpoint; }
            set { _connectedResourceManagementServiceEndpoint = value; }
        }
        ...
        {
            get { return _xiServerInfo.ServerEntry; }
        }
        ...
        private ICommunicationObject? ResourceManagementChannel
        {
            get { return _iResourceManagement as ICommunicationObject; }
        }
        ...
        /// </summary>
        private ServiceEndpoint _connectedResourceManagementServiceEndpoint;
