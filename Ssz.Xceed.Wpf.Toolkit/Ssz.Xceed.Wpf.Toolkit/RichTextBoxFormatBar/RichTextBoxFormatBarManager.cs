...
namespace Ssz.Xceed.Wpf.Toolkit
{
    public class RichTextBoxFormatBarManager : DependencyObject
    {
        ...
        public static readonly DependencyProperty FormatBarProperty = DependencyProperty.RegisterAttached("FormatBar",
            typeof(IRichTextBoxFormatBar), typeof(RichTextBox), new PropertyMetadata(null, OnFormatBarPropertyChanged));
        ...
        private void RichTextBox_MouseButtonUp(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                var selectedText = new TextRange(_richTextBox.Selection.Start, _richTextBox.Selection.End);
#if !VS2008
        ...
        private void OnPreviewMouseMoveParentWindow(object sender, MouseEventArgs e)
        {
            ...
            else if (p.X < -_hideAdornerDistance || p.X > _adorner.ActualWidth + _hideAdornerDistance ||
                     p.Y < -_hideAdornerDistance || p.Y > _adorner.ActualHeight + _hideAdornerDistance)
            {
                HideAdorner();
            }
        ...
        {
            var mousePosition = Mouse.GetPosition(_richTextBox);

            var left = mousePosition.X;
            var top = mousePosition.Y - 15 - adorningEditor.ActualHeight;

            //top
            if (top < 0) top = mousePosition.Y + 10;
