...
{
    public partial class OpcClientDataAccessProvider : DataAccessProviderBase, IDataAccessProvider
    {
        #region construction and destruction

        public OpcClientDataAccessProvider(ILogger<OpcClientDataAccessProvider> logger, IUserFriendlyLogger? userFriendlyLogger = null) :
            base(new LoggersSet<OpcClientDataAccessProvider>(logger, userFriendlyLogger))
        {
            ...
            if (!_xiServerProxy.ContextExists)
            {
                    ...
                    nowUtc > LastFailedConnectionDateTimeUtc + TimeSpan.FromSeconds(30))
                {
                    try
                    {
                        ...
                        _xiServerProxy.InitiateXiContext(ContextParams, "Ssz.Dcs.Addons.OpcClient", Environment.MachineName, WorkingThreadSafeDispatcher);
