...
namespace Antlr4.Runtime
{
    ...
    /// augmentation or other manipulations on it.
    /// <p>
    /// You can insert stuff, replace, and delete chunks. Note that the designTasks
    ...
    /// check to see if there is an designTask at the current index. If so, the
    /// designTask is done and then normal
    /// <see cref="string"/>
    ...
    /// <p>
    /// Since the designTasks are done lazily at
    /// <see cref="GetText()"/>
    /// -time, designTasks do
    /// not screw up the token index values. That is, an insert designTask at token
    /// index
    /// <c>i</c>
    ...
    /// +1..n-1.</p>
    /// <p>
    /// Because designTasks never actually alter the buffer, you may always get the
    ...
    public class TokenStreamRewriter
    {
        ...
        public class RewriteDesignTask
        {
            ...
            protected internal RewriteDesignTask(ITokenStream tokens, int index)
            {
                // Define the rewrite designTask hierarchy
                this.tokens = tokens;
                this.index = index;
            }

            protected internal RewriteDesignTask(ITokenStream tokens, int index, object text)
            {
                this.tokens = tokens;
                this.index = index;
                this.text = text;
            ...
            /// <summary>Execute the rewrite designTask by possibly adding to the buffer.</summary>
            /// <remarks>
            /// Execute the rewrite designTask by possibly adding to the buffer.
            ...
            public override string ToString()
            {
                ...
                opName = Sharpen.Play.Substring(opName, index + 1, opName.Length);
                return "<" + opName + "@" + tokens.Get(this.index) + ":\"" + text + "\">";
            }
        }

        internal class InsertBeforeOp : TokenStreamRewriter.RewriteDesignTask
        {
        ...
        /// instructions.
        /// </remarks>
        internal class ReplaceOp : TokenStreamRewriter.RewriteDesignTask
        {
        ...
        /// <summary>You may have multiple, named streams of rewrite designTasks.</summary>
        /// <remarks>
        /// You may have multiple, named streams of rewrite designTasks.
        /// I'm calling these things "programs."
        /// Maps String (name) &#x2192; rewrite (List)
        /// </remarks>
        protected internal readonly IDictionary<string, IList<TokenStreamRewriter.RewriteDesignTask>> programs;
        ...
        {
            this.tokens = tokens;
            programs = new Dictionary<string, IList<TokenStreamRewriter.RewriteDesignTask>>();
            programs[DefaultProgramName] = new List<TokenStreamRewriter.RewriteDesignTask>(ProgramInitSize);
        ...
        {
            IList<TokenStreamRewriter.RewriteDesignTask> @is;
            if (programs.TryGetValue(programName, out @is))
            {
                programs[programName] = new List<RewriteDesignTask>(@is.Skip(MinTokenIndex).Take(instructionIndex - MinTokenIndex));
            }
        }
        ...
        {
            TokenStreamRewriter.RewriteDesignTask op = new TokenStreamRewriter.InsertBeforeOp(tokens, index, text);
            IList<TokenStreamRewriter.RewriteDesignTask> rewrites = GetProgram(programName);
        ...
        public virtual void Replace(string programName, int from, int to, object text)
        {
            ...
            TokenStreamRewriter.RewriteDesignTask op = new TokenStreamRewriter.ReplaceOp(tokens, from, to, text);
            IList<TokenStreamRewriter.RewriteDesignTask> rewrites = GetProgram(programName);
            op.instructionIndex = rewrites.Count;
            rewrites.Add(op);
        }
        ...
        protected internal virtual IList<TokenStreamRewriter.RewriteDesignTask> GetProgram(string name)
        {
            IList<TokenStreamRewriter.RewriteDesignTask> @is;
        ...
        private IList<TokenStreamRewriter.RewriteDesignTask> InitializeProgram(string name)
        {
            IList<TokenStreamRewriter.RewriteDesignTask> @is = new List<TokenStreamRewriter.RewriteDesignTask>(ProgramInitSize);
            programs[name] = @is;
            return @is;
        }
        ...
        /// and intervals are still consistent. Includes any designTasks done
        ...
        {
            IList<TokenStreamRewriter.RewriteDesignTask> rewrites;
            ...
            IDictionary<int, TokenStreamRewriter.RewriteDesignTask> indexToOp = ReduceToSingleDesignTaskPerIndex(rewrites);
            // Walk buffer, executing instructions and emitting tokens
            int i = start;
            while (i <= stop && i < tokens.Size)
            {
                TokenStreamRewriter.RewriteDesignTask op;
                ...
                if (op == null)
                {
                    // no designTask at that index, just dump token
            ...
            // execute designTask and skip
            // include stuff after end if it's last index in buffer
            // So, if they did an insertAfter(lastValidIndex, "foo"), include
            // foo if end==lastValidIndex.
            if (stop == tokens.Size - 1)
            {
                // Scan any remaining designTasks after last token
                // should be included (they will be inserts).
                foreach (TokenStreamRewriter.RewriteDesignTask op in indexToOp.Values)
                {
        ...
        /// <summary>
        /// We need to combine designTasks and report invalid designTasks (like
        ...
        /// Return a map from token index to designTask.
        /// </remarks>
        protected internal virtual IDictionary<int, TokenStreamRewriter.RewriteDesignTask> ReduceToSingleDesignTaskPerIndex(IList<TokenStreamRewriter.RewriteDesignTask> rewrites)
        {
            //		System.out.println("rewrites="+rewrites);
            // WALK REPLACES
            for (int i = 0; i < rewrites.Count; i++)
            {
                TokenStreamRewriter.RewriteDesignTask op = rewrites[i];
                if (op == null)
                {
                    continue;
                }
            ...
            {
                TokenStreamRewriter.RewriteDesignTask op = rewrites[i_1];
                if (op == null)
                {
                    continue;
                }
            ...
            IDictionary<int, TokenStreamRewriter.RewriteDesignTask> m = new Dictionary<int, TokenStreamRewriter.RewriteDesignTask>();
            for (int i_2 = 0; i_2 < rewrites.Count; i_2++)
            {
                TokenStreamRewriter.RewriteDesignTask op = rewrites[i_2];
                if (op == null)
                {
                    continue;
                }
                ...
                {
                    throw new InvalidDesignTaskException("should only be one op per index");
                }
                m[op.index] = op;
            }
        ...
        /// <summary>Get all designTasks before an index of a particular kind</summary>
        protected internal virtual IList<T> GetKindOfOps<T>(IList<RewriteDesignTask> rewrites, int before)
        {
