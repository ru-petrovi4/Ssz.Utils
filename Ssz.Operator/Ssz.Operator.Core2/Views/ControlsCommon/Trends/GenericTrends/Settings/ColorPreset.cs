...
public class ColorPreset
{
    ...
    public IBrush? PlotAreaBackground { get; private set; }
    public IBrush? PlotBackground { get; private set; }

    public void Apply(TrendsPlotView plot)
    {
        if (Name == @"Default") return;
        if (plot.Plot is null) return;
    ...
    public override bool Equals(object? obj)
    {
        ...
        return Equals(PlotAreaBackground, that.PlotAreaBackground) &&
               Equals(PlotBackground, that.PlotBackground);
    }

    public override int GetHashCode()
    {
        return (PlotBackground?.GetHashCode() ?? 0) ^ (PlotAreaBackground?.GetHashCode() ?? 0);
    }
