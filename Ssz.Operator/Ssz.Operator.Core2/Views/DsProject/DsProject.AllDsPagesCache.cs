...
namespace Ssz.Operator.Core
{
    public partial class DsProject
    {
            ...
            IProgressInfo? progressInfo, List<string>? errorMessages = null)
        {
            ...
            foreach (DrawingInfo drawingInfo in onDriveDsPageDrawingInfos)
            {
                ...
                else
                {
                    ...
                    dsPageDrawing = await ReadDrawingAsync(drawingInfo.FileFullName, false, false) as DsPageDrawing;
            ...
            if (_allDsPagesCache is not null && _allDsPagesCacheIsChanged && !IsReadOnly)
                try
                {
                    using (var memoryStream = new MemoryStream(10 * 1024 * 1024))
                    {
                        ...
                        using (FileStream fileStream = File.Create(Path.Combine(DsProjectPath, AllDsPagesCacheFileName)))
                        {
        ...
        public async Task<DrawingInfo[]> GetDrawingInfosAsync(FileInfo[] drawingFileFullNames,
            IProgressInfo? progressInfo, List<string>? errorMessages = null)
        {
            if (!IsInitialized) return new DrawingInfo[0];

            var drawingInfos = new List<DrawingInfo>(drawingFileFullNames.Length);

            var i = 0;
            foreach (FileInfo fi in drawingFileFullNames)
            {
                if (progressInfo is not null && i > 0 && i % 10 == 0)
                {
                    progressInfo.ProgressBarCurrentValue += 10;
                ...
                var drawingInfo = await ReadDrawingInfoAsync(fi.FullName, true, errorMessages);
