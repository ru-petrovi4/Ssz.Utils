...
namespace Ssz.Operator.Core
{
    public static partial class DsProjectExtensions
    {
            ...
            CreateDsPagesToolkitOperationOptions toolkitOperationOptions, IProgressInfo progressInfo)
        {
            ...
            var existingTemplateDsPageDrawingFileFullName =
                DsProject.Instance.GetExistingDsPageFileFullNameOrNull(toolkitOperationOptions.TemplateDsPageDrawingFileName);
            if (existingTemplateDsPageDrawingFileFullName is null &&
            ...
            if (existingTemplateDsPageDrawingFileFullName is not null)
            {
                templateDsPageDrawing = await DsProject.ReadDrawingAsync(existingTemplateDsPageDrawingFileFullName, false,
                    true) as DsPageDrawing;
                if (templateDsPageDrawing is null)
                {
                    if (progressInfo is not null)
                        await progressInfo.DebugInfoAsync("Cannot read template dsPage " +
                                                          existingTemplateDsPageDrawingFileFullName);
                    return ToolkitOperationResult.DoneWithErrors;
                }
            }
            ...
            foreach (string imageFileFullName in imageFilesFullNames)
            {
                ...
                try
                {
                    ...
                    await dsProject.SaveUnconditionallyAsync(dsPageDrawing, DsProject.IfFileExistsActions.CreateBackup);
                }
                catch (Exception ex)
                {
            ...
            UpdateDsPagesToolkitOperationOptions toolkitOperationOptions, IProgressInfo? progressInfo)
        {
            ...
            foreach (string imageFileFullName in imageFilesFullNames)
            {
                ...
                {
                    var imageFileInfo = new FileInfo(imageFileFullName);
                    var drawingFileFullName = 
                        Path.Combine(dsProject.DsPagesDirectoryFullName,
                        Path.GetFileNameWithoutExtension(imageFileInfo.Name) +
                        DsProject.DsPageFileExtension);

                    if (!File.Exists(drawingFileFullName))
                    {
                        if (progressInfo is not null)
                    ...
                    var dsPageDrawing = await DsProject.ReadDrawingAsync(drawingFileFullName, false, true) as DsPageDrawing;

                    if (dsPageDrawing is null)
                    {
                        if (progressInfo is not null)
                            await progressInfo.DebugInfoAsync(@"Cannot read drawing: " + drawingFileFullName);
                        DsProject.LoggersSet.Logger.LogWarning(@"Cannot read drawing: " + drawingFileFullName);
                        continue;
                    }
