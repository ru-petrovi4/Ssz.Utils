...
namespace Ssz.Operator.Core.DsShapes
{
    public static class DsShapeBaseExtensions
    {
        ...
        {
            dsShapePoint.Offset(dsShape.LeftNotTransformed, dsShape.TopNotTransformed);

            if (dsShape.IsFlipped || dsShape.AngleInitial != 0.0)
            {
                var center = dsShape.CenterInitialPositionNotRounded;
                dsShapePoint.Offset(-center.X, -center.Y);
                dsShapePoint = dsShape.GetTransformGroup().Transform(dsShapePoint);
                dsShapePoint.Offset(center.X, center.Y);
            }
        ...
        public static Point GetDsShapePoint(this DsShapeBase dsShape, Point drawingPoint)
        {
            ...
            {
                var center = dsShape.CenterInitialPositionNotRounded;
                drawingPoint.Offset(-center.X, -center.Y);
                drawingPoint = dsShape.GetInverseTransformGroup().Transform(drawingPoint);
                drawingPoint.Offset(center.X, center.Y);
            }

            drawingPoint.Offset(-dsShape.LeftNotTransformed, -dsShape.TopNotTransformed);
