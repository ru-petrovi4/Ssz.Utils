...
namespace Ssz.Operator.Core.Drawings
{
    public static class DrawingExtensions
    {
        ...
        public static async Task<object?> GetUnderlyingContentAsync(this DsPageDrawing dsPageDrawing)
        {
            if (dsPageDrawing.UnderlyingXaml.IsEmpty) return null;
            try
            {
                return await XamlHelper.LoadFromXamlWithDescAsync(dsPageDrawing.UnderlyingXaml.Xaml, dsPageDrawing.DrawingFilesDirectoryFullName);
            }
            catch
            {
                return null;
