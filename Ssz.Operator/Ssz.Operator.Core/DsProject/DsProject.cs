...
namespace Ssz.Operator.Core
{
        ...
        IUsedAddonsInfo
    {
        ...
        public static async void CreateNew(string dsProjectFileFullName, DsProjectModeEnum mode,
            IProgressInfo? progressInfo = null)
        {
            Instance.Close();

            Instance.Initialize(dsProjectFileFullName, mode, false, false);
            ...
            try
            {
                string fileFullName = Instance.CsvDbDirectoryInfo?.FullName + @"\" + Instance.DataEngine.ElementIdsMapFileName;
            ...
            {
                FileInfo[] dsPageFileInfos = Instance.DsPagesDirectoryInfo
        ...
        public static async Task<bool> ReadDsProjectFromBinFileAsync(string dsProjectFileFullName,
            DsProjectModeEnum mode,
            bool isReadOnly, bool autoConvert, string constants, IProgressInfo? progressInfo = null)
        {
            try
            {
                if (!File.Exists(dsProjectFileFullName))
                    throw new ShowMessageException(dsProjectFileFullName + @": " +
                                                   Resources.FileOpenErrorFileNotFound);

                Instance.Close();

                Instance.Initialize(dsProjectFileFullName, mode, isReadOnly, autoConvert);

                var dsProjectFileStream = GetStream(dsProjectFileFullName);
                ...
                {
                    DsProject.LoggersSet.Logger.LogError(Resources.NotAllDataWasReadMessage);
                    if (!autoConvert)
                ...
                var allDsPagesCacheFileStream = GetStream(Instance.DsProjectPath + @"\" + AllDsPagesCacheFileName);
                if (allDsPagesCacheFileStream is not null)
                {
                                    ...
                                    using (Block block2 = reader.EnterBlock())
                                    {
                                        if (block2.Version == CurrentAllDsPagesCacheSerializationVersion)
                                        {
                                            ...
                                            for (var i = 0; i < count; i += 1)
                                            {
                                                ...
                                                string fileFullName = Instance.GetFileFullName(name + DsProject.DsPageFileExtension);
                                                var dsPageDrawingInfo =
                                                    new DsPageDrawingInfo(new FileInfo(fileFullName));
                    ...
                    {
                        DsProject.LoggersSet.Logger.LogError(ex, "Cannot read dsProject.index file.");
                    }
                    finally
                    {
        ...
        /// </summary>
        [Browsable(false)]
        public string DsProjectPath
        {
            get
            {
                if (string.IsNullOrEmpty(DsProjectFileFullName)) return "";

                if (Instance.Mode == DsProjectModeEnum.WebPlayMode)
                {
                    var i1 = DsProjectFileFullName!.LastIndexOf('/');
                    return DsProjectFileFullName.Substring(0, i1);
                }

                var i = DsProjectFileFullName?.LastIndexOf(Path.DirectorySeparatorChar);
                if (i >= 0)
                    return DsProjectFileFullName!.Substring(0, i.Value);
                else
                    return "";
            }
        }        
        ...
        [Browsable(false)] public bool IsInitialized => Mode != DsProjectModeEnum.Uninitialized;

        [Browsable(false)] public DsProjectModeEnum Mode { get; private set; }
        ...
        [Browsable(false)] public DrawingsStoreModeEnum DrawingsStoreMode { get; set; }
        ...
        [Browsable(false)] public Settings Settings { get; } = new();

        [Browsable(false)] public string UserName { get; set; } = @"";

        [Browsable(false)] public string UserRole { get; set; } = @"";

        [Browsable(false)] public string DataAccessServerHost { get; set; } = @"";
        ...
        [Browsable(false)]
        public DirectoryInfo? AddonsDirectoryInfo
        {
            get
            {
                if (!IsInitialized) return null;

                DirectoryInfo? addonsDirectoryInfo = null;
                try
                {
                    addonsDirectoryInfo =
                        new DirectoryInfo(DsProjectPath).EnumerateDirectories(@"Addons", SearchOption.TopDirectoryOnly)
                            .FirstOrDefault();
                }
                catch (Exception)
                {
                ...
                if (addonsDirectoryInfo is not null) return addonsDirectoryInfo;
                if (IsReadOnly) return null;
                try
                {
                    return new DirectoryInfo(DsProjectPath).CreateSubdirectory(@"Addons");
                }
                catch (Exception ex)
                {
                    DsProject.LoggersSet.Logger.LogCritical(ex, "Create Subdirectory Addons failed.");
                    return null;
                }
            }
        }
        ...
        [Browsable(false)]
        public DirectoryInfo? DsPagesDirectoryInfo
        {
            get
            {
                if (!IsInitialized) return null;

                DirectoryInfo? dsPagesDirectoryInfo = null;
                try
                {
                    dsPagesDirectoryInfo =
                        new DirectoryInfo(DsProjectPath).EnumerateDirectories(@"Pages",
                ...
                if (dsPagesDirectoryInfo is not null) return dsPagesDirectoryInfo;

                if (IsReadOnly) return null;
                try
                {
                    return new DirectoryInfo(DsProjectPath).CreateSubdirectory(@"Pages");
                }
                catch (Exception ex)
                {
                    DsProject.LoggersSet.Logger.LogCritical(ex, "Create Subdirectory Controls failed.");
                    return null;
                }
            }
        }
        ...
        [Browsable(false)]
        public DirectoryInfo? DsShapesDirectoryInfo
        {
            get
            {
                if (!IsInitialized) return null;

                DirectoryInfo? dsShapesDirectoryInfo = null;
                try
                {
                    dsShapesDirectoryInfo =
                        new DirectoryInfo(DsProjectPath).EnumerateDirectories(@"Shapes",
                                SearchOption.TopDirectoryOnly)
                            .FirstOrDefault();
                }
                catch (Exception)
                ...
                if (dsShapesDirectoryInfo is not null) return dsShapesDirectoryInfo;

                if (IsReadOnly) return null;
                try
                {
                    return new DirectoryInfo(DsProjectPath).CreateSubdirectory(@"Shapes");
                }
                catch (Exception ex)
                {
                    DsProject.LoggersSet.Logger.LogCritical(ex, "Create Subdirectory Shapes failed.");
                    return null;
                }
            }
        }
        ...
        /// </summary>
        [Browsable(false)]
        public DirectoryInfo? CsvDbDirectoryInfo
        {
            get
            {
                if (string.IsNullOrEmpty(DsProjectFileFullName)) return null;

                var csvDbDirectoryInfo = new DirectoryInfo(Path.Combine(DsProjectPath, @"CsvDb"));
                if (csvDbDirectoryInfo.Exists) return csvDbDirectoryInfo;
                if (IsReadOnly) return null;
                ...
                {
                    DsProject.LoggersSet.Logger.LogCritical(ex, "Create Subdirectory CsvDb failed.");
                    return null;
                }
            ...
            where T : DataEngineBase
        {
            ...
            if (dataEngine is null)
                try
                {
                    ...
                    DsProject.LoggersSet.Logger.LogError(message);
        ...
        public bool SaveUnconditionally()
        {
            ...
            if (IsReadOnly)
            {
                DsProject.LoggersSet.Logger.LogWarning("DsProject cannot be saved in ReadOnly mode");
                return false;
            }
            ...
            IProgressInfo? progressInfo)
        {
            ...
            foreach (var index in obsoleteDrawingInfosIndexes)
            {
                ...
                if (drawing is null)
                {
                    errorMessages.Add(drawingInfo.FileInfo.Name + @": " + Resources.DrawingFileOpenErrorGeneral + @" " +
                                      Resources.SeeErrorLogForDetails);
                    continue;
                }
                ...
                {
                    errorMessages.Add(drawingInfo.FileInfo.Name + @": " + Resources.DrawingSaveErrorUnSupportedAddons +
                                      @" (" +
                ...
                if (!Instance.SaveUnconditionally(drawing,
            ...
            if (_allDsShapeDrawingInfos is null)
            {
                ...
                if (dsShapesDirectoryInfo is not null)
                    foreach (FileInfo fi in dsShapesDirectoryInfo.GetFiles(@"*" + DsShapeFileExtension,
                        SearchOption.TopDirectoryOnly))
                    {
                        var drawingInfo = ReadDrawingInfo(fi, false, errorMessages) as DsShapeDrawingInfo;
        ...
        private static string GetDsProjectName(string dsProjectFileFullName)
        {
            if (string.IsNullOrEmpty(dsProjectFileFullName)) return "";
            ...
            {
                var i1 = dsProjectFileFullName.LastIndexOf('/');
                return Path.GetFileNameWithoutExtension(dsProjectFileFullName.Substring(i1 + 1));
            }

            var i = dsProjectFileFullName.LastIndexOf('\\');
            return Path.GetFileNameWithoutExtension(dsProjectFileFullName.Substring(i + 1));
        }

        private void Initialize(string dsProjectFileFullName, DsProjectModeEnum mode, bool isReadOnly,
            bool autoConvert)
        {
            if (IsInitialized) 
            ...
            Name = GetDsProjectName(DsProjectFileFullName);                                 
        ...
        private void GlobalUITimerTimerCallback(object? sender, EventArgs e)
        {
            ...
            if (globalUITimerEvent is not null) globalUITimerEvent(_globalUITimerPhase);
        }
        ...
        private void ReSaveAllDsPageDrawingsUnconditionally()
        {
            var errorMessages = new List<string>();

            foreach (DsPageDrawing dsPageDrawing in AllDsPagesCache.Values.ToArray())
            {
                var drawing = ReadDrawing(new FileInfo(dsPageDrawing.FileFullName), false, true);
                if (drawing is null) continue;
                SaveUnconditionally(drawing, IfFileExistsActions.CreateBackup, false, errorMessages);
            }
        ...
        public class DataEngineInfoItemsSource : IItemsSource
        {
            ...
            public ItemCollection GetValues()
            {
                ...
                foreach (DataEngineBase dataEngine in AddonsManager.GetDataEngines())
