...
public class ResourceOwnerPasswordValidator : IResourceOwnerPasswordValidator
{
    ...
    public async Task ValidateAsync(ResourceOwnerPasswordValidationContext context)
    {
        ...
        try
        {
            if (httpContext is not null && String.IsNullOrEmpty(user))
            {
                List<string?> authorization_Kerberos_GSSAPI_SPNEGO_Headers = httpContext!.Request.Headers[IdentityServerConstants.Header_Authorization_Kerberos_GSSAPI_SPNEGO].ToList();                
                if (authorization_Kerberos_GSSAPI_SPNEGO_Headers.Count == 1)
                {
                    _logger.LogDebug("Parser found Header: Authorization_Kerberos_GSSAPI_SPNEGO");

                    user = (authorization_Kerberos_GSSAPI_SPNEGO_Headers[0] ?? @"").ToLowerInvariant();
            ...
            {
                String ldapHost = ConfigurationHelper.GetValue<string>(_configuration, IdentityServerConstants.ConfigurationKey_ActiveDirectory_Server, @"");
                if (_configurationProcessor is not null)
                    ldapHost = _configurationProcessor.ProcessValue(ldapHost);

                int ldapPort = ConfigurationHelper.GetValue<int>(_configuration, IdentityServerConstants.ConfigurationKey_ActiveDirectory_LdapPort, LdapConnection.DefaultPort);
                int ldapVersion = ConfigurationHelper.GetValue<int>(_configuration, IdentityServerConstants.ConfigurationKey_ActiveDirectory_LdapVersion, LdapConnection.LdapV3);                                       
