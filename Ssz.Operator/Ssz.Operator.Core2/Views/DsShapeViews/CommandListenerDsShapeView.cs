...
namespace Ssz.Operator.Core.DsShapeViews
{
    public class CommandListenerDsShapeView : DsShapeViewBase
    {
            ...
            if (propertyName is null ||
                ...
                if (!VisualDesignMode)
                {
                    if (_conditionalDsCommandViewsCollection is not null)
                        ...
                        {
                            dsCommandView.PropertyChanged -= DsCommandViewOnPropertyChanged;
        ...
        private void CommandsManagerOnGotCommand(Command command)
        {
            ...
            switch (((CommandListenerDsShape) DsShapeViewModel.DsShape).CommandToListenOptions)
            {
                case CommandListenerOptions.ListenOnlyInActiveDsPage:
                    var window = Frame is not null ? Frame.PlayWindow : null;
                    ...
                    if (playDrawingCanvas.IsVisible)
                        if (_commandToListenDsCommandView is not null)
                            Dispatcher.UIThread.InvokeAsync(new Action(_commandToListenDsCommandView.DoCommand));
                    return;
                default:
                    if (_commandToListenDsCommandView is not null)
                        Dispatcher.UIThread.InvokeAsync(new Action(_commandToListenDsCommandView.DoCommand));
        ...
        private void DsCommandViewOnPropertyChanged(object? sender, AvaloniaPropertyChangedEventArgs e)
        {
            if (sender is not null && e.Property == DsCommandView.IsEnabledProperty &&
                    e.OldValue is bool && !(bool)e.OldValue && e.NewValue is bool && (bool)e.NewValue)
