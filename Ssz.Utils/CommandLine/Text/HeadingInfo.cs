    ...
    public class HeadingInfo
    {
        ...
        public static HeadingInfo Default
        {
            get
            {
                ...
                {
                    title = ReflectionHelper.AssemblyFromWhichToPullInformation.GetName().Name ?? @"";
                }
                else
                {
                    title = Path.GetFileNameWithoutExtension(titleAttribute.Title) ?? "";
                }
                var versionAttribute = ReflectionHelper.GetAttribute<AssemblyInformationalVersionAttribute>();
                string version;
                if (versionAttribute == null)
                {
                    version = ReflectionHelper.AssemblyFromWhichToPullInformation.GetName().Version?.ToString() ?? @"";
                }
                else
                {
