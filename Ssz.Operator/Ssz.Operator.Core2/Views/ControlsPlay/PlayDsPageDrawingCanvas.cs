...
namespace Ssz.Operator.Core.ControlsPlay
{
    public class PlayDsPageDrawingCanvas : PlayDrawingCanvas
    {
            ...
            IEnumerable<Tuple<ContextMenuDsShapeView, ICloneable>> infosList)
        {
            ...
            foreach (var i in infosList)
            {
                ...
                if (separatorMenuItemInfo is not null)
                {
                    ...
                    separatorMenuItemInfo.IsVisibleInfo.FallbackValue = false;
                    separator.SetVisibilityBindingOrConst(container, IsVisibleProperty,
                ...
                if (menuItemInfo is not null)
                {
                    ...
                    newMenuItem.SetBindingOrConst(container, HeaderedSelectingItemsControl.HeaderProperty,
                        menuItemInfo.HeaderInfo,
                        BindingMode.OneWay,
                        UpdateSourceTrigger.Default);

                    menuItemInfo.IsVisibleInfo.FallbackValue = false;
                    newMenuItem.SetVisibilityBindingOrConst(container, IsVisibleProperty, menuItemInfo.IsVisibleInfo,
                        false, contextMenuDsShapeView.VisualDesignMode);
                    ...
                    newMenuItem.Click +=
                        (sender, args) =>
                            Dispatcher.UIThread.InvokeAsync(
                                new Action(((DsCommandView) ((Control) sender!).Tag!).DoCommand));
        ...
        {
            bool succeeded = e.TryGetPosition(this, out Point pt);
            if (!succeeded)
                return;
            ...
            {
                var parentWithContextMenu = TreeHelper.FindParent<Control>(this, fe => fe.ContextMenu is not null);
                if (parentWithContextMenu is not null)
                {
                    ContextMenu parentContextMenu = parentWithContextMenu.ContextMenu!;
                    if (parentContextMenu.Items.Count > 0) 
                        parentContextMenu.Open();
                }
