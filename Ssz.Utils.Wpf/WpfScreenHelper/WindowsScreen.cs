...
namespace Ssz.Utils.Wpf.WpfScreenHelper
{
    ...
    public class WindowsScreen
    {
        ...
        private static bool _staticMultiMonitorSupport;

        static WindowsScreen()
        {
            _staticMultiMonitorSupport = NativeMethods.GetSystemMetrics(NativeMethods.SM_CMONITORS) != 0;
        }

        private WindowsScreen(IntPtr monitor)
            : this(monitor, IntPtr.Zero)
        {
        }

        private WindowsScreen(IntPtr monitor, IntPtr hdc)
        {
            if (!_staticMultiMonitorSupport || monitor == (IntPtr)PRIMARY_MONITOR)
            {
        ...
        public static IEnumerable<WindowsScreen> AllScreens
        {
            get
            {
                if (_staticMultiMonitorSupport)
                {
                    var closure = new MonitorEnumCallback();
                    var proc = new NativeMethods.MonitorEnumProc(closure.Callback);
                    NativeMethods.EnumDisplayMonitors(NativeMethods.NullHandleRef, new NativeMethods.COMRECT(), proc, IntPtr.Zero);
                    if (closure.Screens.Count > 0)
                    {
                        return closure.Screens.Cast<WindowsScreen>();
                    }
                }
                return new[] { new WindowsScreen((IntPtr)PRIMARY_MONITOR) };
            }
        }
        ...
        public static WindowsScreen PrimaryScreen
        {
            get
            {
                if (_staticMultiMonitorSupport)
                {
                    return AllScreens.First(t => t.Primary);
                }
                return new WindowsScreen((IntPtr)PRIMARY_MONITOR);
            }
        }
        ...
        public Rect WorkingArea
        {
            get
            {
                if (!_staticMultiMonitorSupport || hmonitor == (IntPtr)PRIMARY_MONITOR)
                {
        ...
        public static WindowsScreen FromHandle(IntPtr hwnd)
        {
            if (_staticMultiMonitorSupport)
            {
                return new WindowsScreen(NativeMethods.MonitorFromWindow(new HandleRef(null, hwnd), 2));
            }
            return new WindowsScreen((IntPtr)PRIMARY_MONITOR);
        }
        ...
        public static WindowsScreen FromPoint(Point point)
        {
            if (_staticMultiMonitorSupport)
            {
                var pt = new NativeMethods.POINTSTRUCT((int)point.X, (int)point.Y);
                return new WindowsScreen(NativeMethods.MonitorFromPoint(pt, MONITOR_DEFAULTTONEAREST));
            }
            return new WindowsScreen((IntPtr)PRIMARY_MONITOR);
        }
        ...
        {
            var monitor = obj as WindowsScreen;
            if (monitor != null)
            {
        ...
        private class MonitorEnumCallback
        {
            ...
            {
                this.Screens.Add(new WindowsScreen(monitor, hdc));
                return true;
            }
        }
    }
