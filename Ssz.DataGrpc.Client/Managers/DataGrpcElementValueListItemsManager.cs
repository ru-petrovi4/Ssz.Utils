...
{
    public class DataGrpcElementValueListItemsManager : DataGrpcElementListItemsManagerBase<DataGrpcElementValueListItem, DataGrpcElementValueList>
    {
        #region construction and destruction

        public DataGrpcElementValueListItemsManager(ILogger<DataGrpcProvider> logger) :
            base(logger, false)
        {
        }
        ...
        /// <param name="ct"></param>
        public void Subscribe(DataGrpcServerManager dataGrpcServerProxy, ICallbackDoer? сallbackDoer,
            ...
            try
            {
                ...
                if (firstTimeDataConnection)
                {
                    try
                    {
                        ...
                        {
                            DataGrpcList = dataGrpcServerProxy.NewElementValueList(null);
                        }                            
                    }
                    catch (Exception)
                    {   
                    ...
                    if (!connectionError && DataGrpcList != null && !DataGrpcList.Disposed)
                    {
                        if (firstTimeDataConnection)
                        {
                            DataGrpcList.InformationReport +=
                                (DataGrpcElementValueList dataList, DataGrpcElementValueListItem[] items,
                                    DataGrpcValueStatusTimestamp[] values) =>
                                {
                                    ...
                                    int i = 0;
                                    foreach (DataGrpcElementValueListItem dataGrpcElementValueListItem in items)
                                    {
                ...
                {
                    Logger.LogWarning(ex, "DataGrpcElementValueListItemsManager.Subscribe exception");
        ...
        public object[]? PollChanges()
        {
            ...
            {
                var changedClientObjs = new List<object>();
                DataGrpcElementValueListItem[] changedDataGrpcElementValueListItems = DataGrpcList.PollDataChanges();
                foreach (DataGrpcElementValueListItem dataGrpcElementValueListItem in changedDataGrpcElementValueListItems)
                {
                    var o = dataGrpcElementValueListItem.Obj as DataGrpcListItemWrapper;
                    if (o == null) throw new InvalidOperationException();
                    foreach (var modelItem in o.ModelItems)
        ...
        public object[] Write(object[] clientObjs, Any[] values, DateTime timestampUtc)
        {
            ...
            foreach (var clientObj in clientObjs)
            {
                ...
                DataGrpcElementValueListItem dataGrpcElementValueListItem = modelItem.DataGrpcListItemWrapper.DataGrpcListItem;
                dataGrpcElementValueListItem.PrepareForWrite(new DataGrpcValueStatusTimestamp(values[i]) { TimestampUtc = timestampUtc });
            }

            IEnumerable<DataGrpcElementValueListItem>? failedItems = null;
            try
            {
