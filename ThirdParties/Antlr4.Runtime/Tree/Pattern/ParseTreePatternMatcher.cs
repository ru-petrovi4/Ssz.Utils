...
namespace Antlr4.Runtime.Tree.Pattern
{
    ...
    public class ParseTreePatternMatcher
    {
        ...
        internal virtual IList<Chunk> Split(string pattern)
        {
            ...
            if (ntags == 0)
            {
                string text = Sharpen.Play.Substring(pattern, 0, n);
                chunks.Add(new TextChunk(text));
            }
            if (ntags > 0 && starts[0] > 0)
            {
                // copy text up to first tag into chunks
                string text = Sharpen.Play.Substring(pattern, 0, starts[0]);
                chunks.Add(new TextChunk(text));
            }
            for (int i_1 = 0; i_1 < ntags; i_1++)
            {
                // copy inside of <tag>
                string tag = Sharpen.Play.Substring(pattern, starts[i_1] + start.Length, stops[i_1]);
                string ruleOrToken = tag;
                string label = null;
                int colon = tag.IndexOf(':');
                if (colon >= 0)
                {
                    label = Sharpen.Play.Substring(tag, 0, colon);
                    ruleOrToken = Sharpen.Play.Substring(tag, colon + 1, tag.Length);
                }
                chunks.Add(new TagChunk(label, ruleOrToken));
                if (i_1 + 1 < ntags)
                {
                    // copy from end of <tag> to start of next
                    string text = Sharpen.Play.Substring(pattern, stops[i_1] + stop.Length, starts[i_1 + 1]);
                    chunks.Add(new TextChunk(text));
                }
            }
            if (ntags > 0)
            {
                ...
                if (afterLastTag < n)
                {
                    ...
                    string text = Sharpen.Play.Substring(pattern, afterLastTag, n);
                    chunks.Add(new TextChunk(text));
                }
            }
