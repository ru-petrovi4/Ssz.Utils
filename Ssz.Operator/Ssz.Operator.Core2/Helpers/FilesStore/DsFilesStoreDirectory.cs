...
namespace Ssz.Dcs.CentralServer.Common
{
    ...
    public static class DsFilesStoreDirectoryExtensions
    {
        public static int GetFilesCount(this DsFilesStoreDirectory dsFilesStoreDirectory)
        {
            int count = dsFilesStoreDirectory.DsFilesStoreFilesCollection.Count;
            foreach (var childDsFilesStoreDirectory in dsFilesStoreDirectory.ChildDsFilesStoreDirectoriesCollection)
            {
                count += GetFilesCount(childDsFilesStoreDirectory);
            }
            return count;
        }
    }
